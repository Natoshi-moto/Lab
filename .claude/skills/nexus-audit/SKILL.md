---
name: nexus-audit
description: Run the read-only R001 Nexus audit against baseline-001 and return hash-bound observations.
...
# Nexus R001 blind audit

1. Read `AGENTS.md`, `AUDIT_START_HERE.md`, `constitution/AUDIT.md`, `docs/CLAUDE_AUDIT_GUIDE.md`, and `operations/audits/AUD-R001-CLAUDE-BLIND/TARGET.json`.
...
```bash
./nexus audit-ingest --audit-id AUD-R001-CLAUDE-BLIND --check-only path/to/observation.json
```

7. Place returned files only under `operations/audits/AUD-R001-CLAUDE-BLIND/inbox/claude/` or a branch named `claude/audit-r001-*`.
