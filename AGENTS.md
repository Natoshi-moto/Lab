...
16. **Public board + safe lab mode:** Human may spam `board/` in public. Experimental thrash uses `lab/*` or `play/*` branches (or `/home/anon/Lab-PLAY`); CI workflow `lab-mode.yml` checks those branches only. Never merge thrash dumps to `main` without Human Gate. Prefer filing board items over inventing parallel root folders.
