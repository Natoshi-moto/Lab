...
Governance weight (`model/governance.py`, repaired under E-003) is modelled as a function *of* economic allocation under one of five explicitly named rules — `none`, `nontransferable_equal`, `nontransferable_proportional`, `token_weighted`, `cap_then_renormalize` — computed **only** for scenarios that opt in via a `governance_rules` manifest field, never assumed by default. `nontransferable_proportional` and `token_weighted` are numerically identical at computation time; they differ only in the *durability* label (frozen at genesis vs. moves with the token on a secondary market).

> **Micro-repair note:** the fifth rule was previously named `continuously_capped`, which could be misread as a hard, final per-holder ceiling. It is not one. `cap_then_renormalize` clips each holder's *raw* proportional weight at `cap_bps` of the pre-cap total, then **renormalizes** the clipped weights so they sum to exactly 1 again — and that renormalization step can push an individual holder's *final* weight back above the nominal clip fraction when several holders are clipped at once (proven at 1/2/3-holder scale in `tests/test_governance.py::TestCapThenRenormalize`). Every scenario report separates the three distinct quantities — `raw_proportional_weights`, `pre_normalization_clipped_weights`, and the final `weights` (aliased `final_normalized_weights` in scenario output) — rather than collapsing them into one "capped" number. This rule reduced concentration below both the simple-majority and blocking-third thresholds in the tested many-holder scenario (`14_governance_cap_then_renormalize`); that is a scenario-specific result, not a durable per-holder guarantee — durable hard-capped governance enforcement is open Track E work (see `NONCLAIMS_AND_OPEN_QUESTIONS.md`).

This lets Track 1's "governance rights proportional to allocation versus independently capped" be tested as an orthogonal axis rather than a seventh allocation scheme.
