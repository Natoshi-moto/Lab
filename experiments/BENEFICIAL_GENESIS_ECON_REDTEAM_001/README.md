...
**Follow-up micro-repair:** (1) added a shared, fail-closed participant-input validator that every public function in `model/allocation.py` calls directly (duplicate/empty/non-string IDs, boolean/non-integer/negative weights, invalid pool/cap/winner-count values), with direct tests against the public functions themselves rather than relying only on `scenario.py`'s own checks; (2) renamed the `continuously_capped` governance rule to `cap_then_renormalize` and now report raw proportional weights, pre-normalization clipped weights, and final normalized weights separately, since renormalization can push a holder's final share back above the nominal clip fraction (proven at 1/2/3-holder scale in `tests/test_governance.py`) — this rule is described only as having reduced concentration in the tested many-holder scenario, never as a hard final per-holder cap; (3) replaced the receipt's self-referential `branch_tip` field with explicit `repair_logic_commit`/`receipt_binding_parent_commit`/`receipt_self_binding_note` fields.
...
5. Governance capture by donation size alone is conditional on the integration rule: `none`/`nontransferable_equal` grant no capture; `token_weighted`/`nontransferable_proportional` let a single honest whale cross simple majority (52.488%, `13_governance_rules_comparison`); a cap-then-renormalize rule (`14_governance_cap_then_renormalize`) reduced concentration below the majority/blocking-third thresholds in the tested 501-donor population, though the whale's own final share (9.8%/17.8% at the 5%/10% nominal clip) already exceeds the nominal clip fraction — this is **not** a hard per-holder cap, and durable, continuously-enforced governance caps remain open Track E work. **Conditional (FC3).**
