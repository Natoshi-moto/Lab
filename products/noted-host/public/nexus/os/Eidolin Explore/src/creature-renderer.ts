...
function drawCreatureBody(
  ...
  drawCreatureShadow(ctx, sx, sy, traits);
  drawCreatureAura(ctx, sx, sy, traits, anim.lastUpdateTime, opts.emergence);
  ...
  drawTendrils(ctx, traits, anim, anim.lastUpdateTime);
  // Orbitals sit behind and beside the silhouette — alive, but not noisy.
  drawOrbitalMotes(ctx, traits, anim.lastUpdateTime);
  // Body
  drawBody(ctx, traits, anim.lastUpdateTime);
  // Limbs (in front of body for legibility)
  drawLimbs(ctx, traits, gaitPhase);
  // Eyes (frontmost)
  drawEyes(ctx, traits, anim.lastUpdateTime);
...
function drawCreatureAura(
  ...
  const radius = Math.max(traits.bodyW, traits.bodyH) * (1.15 + traits.translucency * 0.65);
  const pulse = 0.72 + Math.sin(time * 1.7 + traits.gaitOffset * Math.PI * 2) * 0.08;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  ctx.globalAlpha *= (0.16 + traits.translucency * 0.18) * (0.55 + emergence * 0.45);
  const grad = ctx.createRadialGradient(sx, sy, radius * 0.12, sx, sy, radius * 1.35 * pulse);
  grad.addColorStop(0, traits.accentColor.replace('hsl(', 'hsla(').replace(')', ', 0.42)'));
  grad.addColorStop(0.45, traits.color.replace('hsl(', 'hsla(').replace(')', ', 0.14)'));
  grad.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = grad;
  ctx.beginPath();
  ctx.ellipse(sx, sy, radius * 1.25, radius * 0.92, -0.1, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}


function drawCreatureShadow(
  ctx: CanvasRenderingContext2D,
  sx: number,
  sy: number,
  traits: RenderTraits,
  ...
  ctx.save();
  const width = traits.bodyW * 1.18;
  const height = 8 + traits.bodyH * 0.10;
  const grad = ctx.createRadialGradient(sx, sy + traits.bodyH * 0.42, 0, sx, sy + traits.bodyH * 0.42, width);
  grad.addColorStop(0, 'rgba(0,0,0,.28)');
  grad.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = grad;
  ctx.beginPath();
  ctx.ellipse(sx, sy + traits.bodyH * 0.44, width, height, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
}
...
function drawBody(ctx: CanvasRenderingContext2D, traits: RenderTraits, time: number): void {
  const w = traits.bodyW;
  const h = traits.bodyH;
  const taperFactor = traits.taper;

  ctx.save();
  // Translucency: lower alpha for "glassy" creatures. The body dims,
  // but eyes and rings remain readable because alpha is restored.
  ctx.globalAlpha *= 1 - traits.translucency * 0.5;

  const grad = ctx.createLinearGradient(-w * 0.35, -h * 0.55, w * 0.35, h * 0.55);
  grad.addColorStop(0, traits.accentColor);
  grad.addColorStop(0.38, traits.color);
  grad.addColorStop(1, 'rgba(10, 14, 16, .92)');
  ctx.fillStyle = grad;
  ...
  ctx.fillStyle = '#fff';
  ctx.beginPath();
  ctx.ellipse(-w * 0.16, -h * 0.23, w * 0.20, h * 0.08, -0.25, 0, Math.PI * 2);
  ctx.fill();

  // Soft inner veins: three low-alpha deterministic arcs that make the
  // creature read as translucent biology rather than a flat token.
  ctx.globalAlpha *= 0.55;
  ctx.strokeStyle = traits.accentColor;
  ctx.lineWidth = 0.85;
  for (let i = 0; i < 3; i++) {
    const phase = time * (0.45 + i * 0.11) + traits.gaitOffset * 6 + i;
    const y = -h * 0.05 + (i - 1) * h * 0.15;
    ctx.beginPath();
    ctx.moveTo(-w * 0.32, y);
    ctx.bezierCurveTo(
      -w * 0.12, y + Math.sin(phase) * h * 0.10,
       w * 0.10, y + Math.cos(phase * 0.8) * h * 0.08,
       w * 0.34, y + Math.sin(phase + 1.7) * h * 0.09,
    );
    ctx.stroke();
  }
...
function drawEyes(ctx: CanvasRenderingContext2D, traits: RenderTraits, time: number): void {
  const eyeY = -traits.bodyH * 0.18;
  const spread = traits.bodyW * 0.55;
  const count = traits.eyeCount;

  const blinkPhase = mod(time * 0.18 + traits.gaitOffset, 1);
  const blink = blinkPhase > 0.93 ? 1 - Math.sin(((blinkPhase - 0.93) / 0.07) * Math.PI) * 0.82 : 1;
  const pupilDrift = Math.sin(time * 0.55 + traits.gaitOffset * 5) * traits.eyePixelSize * 0.25;
  ...
  for (let i = 0; i < count; i++) {
    ...
    ctx.fillStyle = '#fff';
    ctx.beginPath();
    ctx.ellipse(eyeX, eyeY, traits.eyePixelSize, traits.eyePixelSize * blink, 0, 0, Math.PI * 2);
    ctx.fill();
    ...
    if (blink > 0.12) {
      ...
      ctx.fillStyle = '#000';
      ctx.beginPath();
      ctx.ellipse(eyeX + pupilDrift, eyeY, traits.eyePixelSize * 0.5, traits.eyePixelSize * 0.5 * blink, 0, 0, Math.PI * 2);
      ctx.fill();

      // Tiny catchlight: cheap, but makes each creature read as alive.
      ctx.fillStyle = 'rgba(255,255,255,.82)';
      ctx.beginPath();
      ctx.arc(eyeX - traits.eyePixelSize * 0.22, eyeY - traits.eyePixelSize * 0.24, Math.max(1, traits.eyePixelSize * 0.18), 0, Math.PI * 2);
      ctx.fill();
    }
  }
}
...
function drawTendrils(
  ...
  for (let t = 0; t < traits.tendrilCount; t++) {
    ...
    ctx.shadowColor = traits.accentColor;
    ctx.shadowBlur = 5 + Math.sin(time * 1.7 + t) * 1.5;
    ctx.stroke();
    ctx.shadowBlur = 0;

    const tip = segments[TENDRIL_SEGMENTS - 1];
    const tipPulse = 0.75 + Math.sin(time * 2.2 + t * 1.7) * 0.25;
    ctx.save();
    ctx.globalCompositeOperation = 'screen';
    ctx.fillStyle = traits.accentColor.replace('hsl(', 'hsla(').replace(')', ', 0.38)');
    ctx.beginPath();
    ctx.arc(curX + tip.x, curY + tip.y, 2.6 * tipPulse, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
}

function drawOrbitalMotes(ctx: CanvasRenderingContext2D, traits: RenderTraits, time: number): void {
  const count = Math.max(3, Math.min(8, Math.floor(traits.eyeCount + traits.tendrilCount / 2)));
  const rx = traits.bodyW * 0.72;
  const ry = traits.bodyH * 0.50;
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  for (let i = 0; i < count; i++) {
    const phase = time * (0.28 + i * 0.025) + traits.gaitOffset * 6 + i * Math.PI * 2 / count;
    const x = Math.cos(phase) * rx;
    const y = Math.sin(phase * 1.13) * ry - traits.bodyH * 0.02;
    const front = Math.sin(phase) > -0.15;
    ctx.globalAlpha = front ? 0.26 : 0.10;
    ctx.fillStyle = front ? traits.accentColor : traits.color;
    ctx.beginPath();
