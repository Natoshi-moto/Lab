...
function drawCeremonyVeil(
  ...
  const grad = ctx.createRadialGradient(
    ...
    dims.width / 2,
    dims.height * 0.58,
    Math.max(dims.width, dims.height) * 0.75,
  );
  grad.addColorStop(0, `rgba(18, 22, 26, ${alpha * 0.72})`);
  grad.addColorStop(0.58, `rgba(8, 10, 16, ${alpha})`);
  grad.addColorStop(1, `rgba(0, 0, 0, ${Math.min(0.96, alpha + 0.22)})`);
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, dims.width, dims.height);

  // Faint ceremony lattice: purely visual, deterministic by index. It gives
  // the Hush a held-breath geometry without changing any phase logic.
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  const cx = dims.width / 2;
  const cy = dims.height * 0.55;
  const count = 12;
  for (let i = 0; i < count; i++) {
    const a = (i / count) * Math.PI * 2 + elapsed * 0.055;
    const r1 = dims.height * (0.18 + (i % 3) * 0.03);
    const r2 = dims.height * (0.34 + (i % 4) * 0.025);
    ctx.strokeStyle = `rgba(210,255,235,${alpha * (0.020 + (i % 2) * 0.018)})`;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(cx + Math.cos(a) * r1, cy + Math.sin(a) * r1 * 0.64);
    ctx.lineTo(cx + Math.cos(a + 0.42) * r2, cy + Math.sin(a + 0.42) * r2 * 0.64);
    ctx.stroke();
  }
  ctx.restore();
}
...
function drawVesselSilhouette(
  ...
  ctx.save();
  const aura = ctx.createRadialGradient(cx, top + h * 0.55, 0, cx, top + h * 0.55, w * 1.55);
  aura.addColorStop(0, 'rgba(220,255,238,.20)');
  aura.addColorStop(0.55, 'rgba(120,190,220,.08)');
  aura.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = aura;
  ctx.beginPath();
  ctx.arc(cx, top + h * 0.55, w * 1.55, 0, Math.PI * 2);
  ctx.fill();

  const shell = ctx.createLinearGradient(cx, top, cx, top + h);
  shell.addColorStop(0, fill === '#000' ? 'rgba(0,0,0,.94)' : fill);
  shell.addColorStop(0.48, fill);
  shell.addColorStop(1, 'rgba(4,6,8,.98)');
  ctx.fillStyle = shell;
  ctx.beginPath();
  ...
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = 'rgba(238,255,245,.26)';
  ctx.lineWidth = 1.2;
  ctx.stroke();
  ctx.restore();
}
...
function drawGestation(
  ...
  // Rising spiral strands inside the vessel. They are phase-locked to the
  // gestation interval, so the fill color still remains the main beat.
  ctx.globalCompositeOperation = 'screen';
  ctx.strokeStyle = dnaColor(state.offspring.dna, 0.34);
  ctx.lineWidth = 1.2;
  for (let i = 0; i < 5; i++) {
    const phase = t * Math.PI * 2 + i * 1.25;
    ctx.beginPath();
    for (let s = 0; s <= 18; s++) {
      const u = s / 18;
      const y = v.bottom - v.h * u;
      const amp = v.w * (0.10 + u * 0.17);
      const x = v.cx + Math.sin(phase + u * 5.4) * amp;
...
function drawCrack(
  ...
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  const flashRadius = v.w * (0.8 + smoothstep(t) * 1.25);
  const flash = ctx.createRadialGradient(v.cx, v.top + v.h / 2, 0, v.cx, v.top + v.h / 2, flashRadius);
  flash.addColorStop(0, `rgba(255,255,255,${0.20 + t * 0.20})`);
  flash.addColorStop(0.44, dnaColor(state.offspring.dna, 0.22));
  flash.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = flash;
  ctx.beginPath();
  ctx.arc(v.cx, v.top + v.h / 2, flashRadius, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
...
function drawNaming(
  ...
  // Offspring silhouette in DNA color: a newborn icon, not a generic disc.
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  const halo = ctx.createRadialGradient(cx, cy, 0, cx, cy, v.w * 1.35);
  halo.addColorStop(0, dnaColor(state.offspring.dna, 0.55));
  halo.addColorStop(0.52, dnaColor(state.offspring.dna, 0.18));
  halo.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = halo;
  ctx.beginPath();
  ctx.arc(cx, cy, v.w * 1.35, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();
  ...
  ctx.lineWidth = 1.4;
  ctx.beginPath();
  ctx.ellipse(cx, cy, v.w * 0.42, v.w * 0.32, -0.08, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  const eyeY = cy - v.w * 0.06;
  ctx.fillStyle = 'rgba(255,255,255,.88)';
  ctx.beginPath(); ctx.arc(cx - v.w * 0.14, eyeY, v.w * 0.045, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + v.w * 0.14, eyeY, v.w * 0.045, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = 'rgba(0,0,0,.86)';
  ctx.beginPath(); ctx.arc(cx - v.w * 0.14, eyeY, v.w * 0.022, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.arc(cx + v.w * 0.14, eyeY, v.w * 0.022, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
