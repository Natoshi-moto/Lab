  ...
  if (!hud) {
    ...
    hud.innerHTML = `
      <section class="hud-shell" aria-live="polite">
        ...
        <div class="hint">←/→ or A/D move · B battle · V vessel · D debug · Space skips ceremony</div>
      </section>
    `;
...
function update(state: RuntimeState, input: InputState, dt: number): RuntimeState {
  ...
  if (!next.battle && !next.ceremony) {
    const targetSpeed = input.left ? -MAX_SPEED : input.right ? MAX_SPEED : 0;
    next = {
      ...next,
...
function boot(): void {
  ...
  const key = (e: KeyboardEvent, down: boolean) => {
    ...
    if (down && e.key.toLowerCase() === 'd') state = { ...state, debug: !state.debug };
  ...
  const frame = (t: number) => {
    ...
    draw(ctx, dims, state);
    if (status) {
      status.textContent = `${state.message} · X=${Math.round(state.playerX)} · ${state.active?.state ?? 'none'} · ${state.active?.slot.temperament ?? ''}`;
    }
