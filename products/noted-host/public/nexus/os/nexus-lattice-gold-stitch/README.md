...
- Added a judge-friendly Auto demo mode that walks the slice from manifestation read → battle response → Vessel birth.
- Fixed the D-key conflict: A/D still move, while F3/backtick toggles debug.
...
Then open `http://localhost:5173/`. Press **Auto demo** for the guided judge path; use **F3** or the HUD button for debug.
