...
The latest pass adds two concrete judge-facing upgrades:

- **Auto demo path:** a reviewer can click one control and watch the runtime move toward the active slot, select sensible battle responses, and open the Vessel after stillness.
- **Control hygiene:** the original HUD hint made `D` both movement and debug. The build now keeps A/D movement intact and moves debug to F3/backtick plus the HUD button.

These changes matter because they reduce demonstration friction without weakening the deterministic proof surface.
